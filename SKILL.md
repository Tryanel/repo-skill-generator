...
description: Inspect a code repository and create repo-specific Codex, Claude, or OpenCode skills that distill its capabilities into portable knowledge packs. Use when an agent needs to learn what focused directories or scripts do, capture workflows, templates, data transformations, integrations, inputs, outputs, and tests, then generate a platform-specific skill that can recreate equivalent standalone scripts without depending on the original repository checkout.
---
...
1. Confirm the target repository path, target platform, and intended output skill name. Normalize the skill name to lowercase hyphen-case.
   ...
   Also capture any operations the user wants turned into callable scripts under the generated skill's `scripts/` directory.
...
into callable helpers bundled with the generated skill:

```bash
python scripts/draft_repo_skill.py --repo /path/to/repo --skill-name mining-dev --target all --focus dag --focus plugin/templates --script-focus dag --script-focus plugin/templates --script-note "turn the DAG runner and database template renderer into portable scripts" --output repo-skill-draft.md
```
...
6. Fill the generated bundled references with self-contained repository facts:
   ...
   - `references/implementation-blueprint.md`: how to recreate equivalent standalone scripts without importing the original repository.
   - `references/callable-scripts.md`: requested script contracts, evidence paths, and script starter shapes when the user asks for generated helper scripts.
...
8. Write the generated skill with progressive disclosure:
   ...
   - Add scripts only for stable, repeatable, user-requested repository capabilities. Generated scripts must be implemented, runnable with `--help`, and tested before the skill is shared.
   - Add assets only when future agents must copy or reuse concrete files.
9. Validate the generated skill with the target platform's validation rules when available, and test any bundled scripts.

## Portability Requirements

- The generated skill must be useful after copying it to another machine or teammate.
- Bundle enough capability, source-map, and implementation knowledge that future agents can write equivalent standalone scripts without the original local checkout used during generation.
- Future agents should use the bundled capability map, source map, and implementation blueprint first. They may open a checkout only to verify drift or inspect details missing from the bundled knowledge pack.
- If the user asks for callable scripts, bundle the completed scripts under `scripts/` and document their contracts in `references/callable-scripts.md`. Do not ship placeholder scripts or scripts that import the original repository unless the user explicitly asked for wrappers.
...
- For "out of the box" portability, ship `capability-map.md`, `repo-conventions.md`, `source-map.md`, `implementation-blueprint.md`, `task-playbook.md`, and any requested `scripts/` together.
- If the user says certain directories are core, pass them through `--focus` and preserve the rationale in bundled references.
- If the user names paths, functions, or operations that should become tools, pass them through `--script-focus` or record them in `--script-note`, then implement the final scripts from the evidence before distribution.
...
- `scripts/draft_repo_skill.py`: Scans a repository for text-like docs, manifests, source files, tests, config clues, user-focused capability directories, and optional script targets, then writes a Markdown draft for Codex, Claude, OpenCode, or all target skill formats.

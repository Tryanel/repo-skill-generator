...
description: Inspect a code repository and create repo-specific Codex, Claude, OpenCode, or Agent Skills-compatible instructions that distill focused capabilities into portable knowledge packs and optional standalone scripts.
---
...
1. Confirm the target repository path, target platform, output skill name, user-defined focus paths, and any capabilities the user wants turned into callable scripts.
...
Use `--script-focus PATH_OR_LABEL` and `--script-note TEXT` when the user wants specific paths, functions, or operations turned into bundled helper scripts.
...
5. Put core workflow in `SKILL.md`; move self-contained repository knowledge into `references/capability-map.md`, `references/repo-conventions.md`, `references/source-map.md`, `references/implementation-blueprint.md`, and `references/task-playbook.md`.
6. When scripts are requested, implement tested files under `scripts/` and document contracts in `references/callable-scripts.md`. Do not ship placeholder scripts.
...
- For out-of-the-box use, ship capability map, conventions, source map, implementation blueprint, task playbook, and any requested scripts together.
